...
int WatershedDelineation::run_watershed_delineation(const std::vector<PourPoint>& pour_points_input)
{
    ...
    try {
        ...
        extract_stream_network(accumulation, stream_grid, static_cast<double>(params.stream_threshold));
        ...
        richdem::Array2D<double> masked_accumulation;
        watershed_masking::mask_array_with_watershed(watershed_label, accumulation, masked_accumulation);
        masked_accumulation.geotransform = accumulation.geotransform;
        masked_accumulation.projection = accumulation.projection;
        masked_accumulation.saveGDAL(params.output_prefix + "_FAC_masked.tif");

        richdem::Array2D<int> masked_flowdirs;
        watershed_masking::mask_array_with_watershed(watershed_label, flowdirs_orig, masked_flowdirs);
        masked_flowdirs.geotransform = flowdirs_orig.geotransform;
        masked_flowdirs.projection = flowdirs_orig.projection;
        masked_flowdirs.saveGDAL(params.output_prefix + "_DIR_masked.tif");

        richdem::Array2D<uint8_t> masked_streams;
        watershed_masking::mask_array_with_watershed(watershed_label, stream_grid, masked_streams);
        masked_streams.geotransform = stream_grid.geotransform;
        masked_streams.projection = stream_grid.projection;
        masked_streams.saveGDAL(params.output_prefix + "_streams_masked.tif");
